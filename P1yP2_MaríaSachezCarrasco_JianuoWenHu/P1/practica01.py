...
from VectorialRegression import v_cost_test_obj
from VectorialRegression import v_compute_gradient_obj
...
def test_gradient_descent(x_train, y_train, w, b):
    ...
    result = lr.gradient_descent(0.01, 1500)
    w = result[0]
    b = result[1]
    predict1 = 3.5 * w + b
...
def test_gradient_v(x_train, y_train):
    initial_w = 0
    initial_b = 0

    lr = VectorialReg(x_train,y_train,initial_w,initial_b)
    lr_tmp_dj_dw, lr_tmp_dj_db = lr.v_compute_gradient()
    print('Gradient at initial w, b (similar to)(-50.498559851122586 -6.988606075159082):',
          lr_tmp_dj_dw, lr_tmp_dj_db)
    ...
    lr = VectorialReg(x_train,y_train,test_w,test_b)
    lr_tmp_dj_dw, lr_tmp_dj_db = lr.v_compute_gradient()
    print('Gradient at initial w, b (similar to)(-38.7005262336415 -5.369312042261976):',
          lr_tmp_dj_dw, lr_tmp_dj_db)

    print("----Compute gradient POO-------")
    compute_gradient_test(v_compute_gradient_obj)

def test_cost_v(x_train, y_train):

    initial_w = 2
    initial_b = 1

    lr = VectorialReg(x_train,y_train,initial_w,initial_b)
    lrcost = lr.v_compute_cost()
    print(type(lrcost))
    print(f'Cost at initial w (35.841): {lrcost:.3f}')

    print("----Compute cost POO-------")
    compute_cost_test(v_cost_test_obj)



def test_gradient_descent_v(x_train, y_train, w, b):
    lr = VectorialReg(x_train,y_train,w,b)
    result = lr.v_gradient_descent(0.01, 1500)
    w = result[0]
    b = result[1]
    predict1 = 3.5 * w + b
    print('for score 3.5, we predict user score of (3.97) %.2f' %
