...
class VectorialReg:
    ...
    """
    def v_gradient_descent(self, alpha, num_iters):
        ...
        for i in range(0, num_iters):
            gradientw, gradientb = self.v_compute_gradient()
            J_history.append(v_cost_test_obj(self.x, self.y, self.w, self.b))
...
def v_cost_test_obj(x,y,w_init,b_init):
    lr = VectorialReg(x,y,w_init,b_init)
    cost = lr.v_compute_cost()
    return cost

def v_compute_gradient_obj(x,y,w_init,b_init):
    lr = VectorialReg(x,y,w_init,b_init)
    dw,db = lr.v_compute_gradient()
    return dw,db
